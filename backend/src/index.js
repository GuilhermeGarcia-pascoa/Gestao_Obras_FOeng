...
const syncRouter       = require('./routes/sync');                      // ← NOVO
const { exportarExcel, exportarPdf } = require('./routes/export');
const { auth, soGestor } = require('./middleware/auth');
const { iniciarSyncAutomatico } = require('./services/syncFoPanel');   // ← NOVO
...
app.use('/api/sync',       syncRouter);                                // ← NOVO
  ...
  Comandos disponíveis:
    ...
    exit    — Encerrar
      ...
      case 'env':
        ...
        console.log(`  FOPANEL_HOST = ${process.env.FOPANEL_HOST || '(não definido)'}`);
